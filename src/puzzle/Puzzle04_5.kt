...
/**
 * 팩토리 메소드는 single method가 여러 종류의 객체를 생성하는 것이고
 * 추상 팩토리는 object 별 한 종류의 객체를 생성하는데, 여러 객체를 생성해서 여러 종류의 객체를 생성하는 factory 효과를 나타내는 것이다.
 *
 * 팩토리 메소드 패턴은 조건에 따른 객체 생성을 팩토리 클래스로 위임하여, 팩토리 클래스에서 객체를 생성하는 패턴
 * 추상 팩토리 패턴은 서로 관련이 있는 객체들을 통째로 묶어서 팩토리 클래스로 만들고, 이들 팩토리를 조건에 따라 생성하도록 다시 팩토리를 만들어서 객체를 생성하는 패턴
 */
class Puzzle04_5 {
